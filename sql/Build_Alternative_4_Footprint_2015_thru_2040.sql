...
USE [gis]
Go
Drop view Analysis.Alt_4_GrowthFootprint
Go
create view Analysis.Alt_4_GrowthFootprint as
SELECT
--Using Distinct due to duplicate parcels in the Parcel Geometry Table
Distinct
Top 20000
p.OBJECTID, 
p.COUNTY_ID, 
...
FROM            
Analysis.RUN7224_PARCEL_DATA_DIFF AS alt_4_Diff INNER JOIN
Analysis.URBANSIM_PARCELS AS p ON alt_4_Diff.parcel_id = p.PARCEL_ID RIGHT OUTER JOIN
...
Order By County_ID
Go
Drop View Analysis.Alt_4_GrowthFootprint_Features 
...
Go
create view Analysis.Alt_4_GrowthFootprint_Features as
SELECT        
OBJECTID, 
COUNTY_ID, 
parcel_id, 
...
--Features
geometry::STGeomFromText(Features,26910).MakeValid() as Shape
FROM            Analysis.Alt_4_GrowthFootprint
WHERE        (People_Per_Acre >= 8) OR
                         (Jobs_Per_Acre >= 10)
Go
select * From Analysis.Alt_4_GrowthFootprint_Features
